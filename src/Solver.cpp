...
Solver::Solver(primitive_t primitive_A, primitive_t primitive_B)
    : FiniteDifference(Objective::get_primitives_description(primitive_A, primitive_B) + "-Solver"), objective(primitive_A, primitive_B) {}
...
void Solver::compute_dtds(MatrixXd& dtds, const VectorXd& s, const VectorXd& t) const {
    ...
    objective.compute_p2OpT2(p2OpT2, s, t);
    objective.compute_p2OpTpS(p2OpTpS, s, t);
...
bool Solver::are_t_and_s_in_sync(const VectorXd& s, const VectorXd& t) const {
    ...
    objective.compute_pOpT(grad, s, t);
    if (grad.norm() > 1e-6) {
